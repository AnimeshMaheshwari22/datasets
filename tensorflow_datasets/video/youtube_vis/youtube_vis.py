  ...
  def __init__(self,
    ...
    Args:
      ...
      split_train_data_range: If not None, must be a tuple of two integers
        indicating the start and end of the train data to subsample into a
        manufactured training split. Because the default validation and test
        splits contain no labels, this allows for validation and
      ...
      split_val_data_range: If not None, must be a tuple of integers indicating
        the start and end of the train data to subsample into a manufactured
        validation split. Because the default validation and test
        splits contain no labels, this allows for validation and
        testing without uploading to the youtube_vis third party servers.
      split_test_data_range: If not None, must be a tuple of integers indicating
        the start and end of the train data to subsample into a manufactured
        test split. Because the default validation and test splits contain no
        labels, this allows for validation and testing without uploading to the
        youtube_vis third party servers.
...
class YoutubeVis(tfds.core.BeamBasedBuilder):
  ...
  def _generate_examples(self, annotations, all_frames,
    ...
    if video_range_to_use is not None:
      # Use a restricted range of the videos.
      assert 0 <= video_range_to_use[0] <= len(video_keys)
